...
const Cart = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flext-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-[100%] py-6 gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className=" flex flex-col w-full items-start justify-start gap-3 h-[65%] overflow-y-auto scrollbar-none px-4 text-primary">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
            ...
            <div className=" rounded-[60px] w-full h-[20%] flex flex-col items-center justify-center px-4 py-6 gap-20">
...
export const CartItemCard = ({ index, data }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full flex items-center justify-start bg-zinc-800 rounded-xl drop-shadow-md px-4 gap-4"
    >
      <img
        src={data?.imageURL}
